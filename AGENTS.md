...
- Never run pytest with relative `--basetemp` inside the workspace (e.g. `--basetemp=.tmp`). Always
  use the OS temp path (`%TEMP%`) or run `./scripts/check.ps1`. Local temp directories created in
  sandbox environments or permission tests can create restricted Windows ACLs that trigger
  `Permission denied` errors in Git.
