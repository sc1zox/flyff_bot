...
- [ ] Connects `FrameProvider`, `MobDetector`, `TargetVerifier`, and `LootLogReader` into a unified `PerceptionPipeline`.
- [ ] Aggregates detections into a new `WorldState` instance on each tick (player status, selected target, visible mobs, recent loot).
- [ ] Emits state change events/signals when target transitions occur or new mobs appear.
- [ ] Handles individual component failures gracefully (e.g. OCR timeout does not crash mob detection).
- [ ] Automated unit tests verify end-to-end perception aggregation with mock feeds.
