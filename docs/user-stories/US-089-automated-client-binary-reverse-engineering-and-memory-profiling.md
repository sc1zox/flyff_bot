...
- Builds upon memory reader implementations in [US-053](completed/US-053-pure-gps-navigation-and-client-profile-configuration.md) (`LivePositionReader`), [US-056](completed/US-056-client-camera-state-and-projection-matrix-reader.md) (`LiveCameraReader`), [US-063](completed/US-063-client-dungeon-data-and-live-cooldown-memory-extraction.md) (`LiveDungeonCooldownReader`), and [US-076](completed/US-076-read-only-client-player-stats-memory-reader.md) (`LivePlayerStatsReader`).
- The reverse engineering process follows deterministic static binary analysis of the PE executable file on disk:
  ...
  2. **MSVC RTTI & VTable Resolution**: Scans `.rdata` for TypeDescriptors (`.?AVCMover@@`, `.?AVCWndDungeonCooldownList@@`, `.?AVCPlayerDataCenter@@`, `.?AVCWndStatus@@`) and traverses `_RTTICompleteObjectLocator` descriptors to locate authoritative virtual method tables.
...
- [ ] Given a local target client directory containing `neuz.exe`, when the automated binary profiler is executed, then it parses the PE headers and scans section byte streams without external disassembler dependencies (e.g. standard library PE/x86-64 decoder).
- [ ] Given MSVC RTTI symbols are present in `.rdata`, when the profiler executes, then it authoritatively identifies the VTables for `CMover`, `CWndStatus`, and `CWndDungeonCooldownList`.
- [ ] Given `GetPlayer()` instruction sequences are identified, when displacement calculations are evaluated, then the profiler extracts the exact `g_pPlayer` pointer RVA (`0x00B7C908`) and validates the local player coordinate offset (`+0x188`).
- [ ] Given HUD calculation routines are analyzed, when mathematical operations (`imul`, `idiv`, `cvtsi2ss`) are decoded, then the profiler extracts the player stat field offsets (`hp`, `mp`, `fp`, `level`, `exp`) and generates a typed `ClientPlayerStatsProfile`.
- [ ] Given Direct3D matrix transformation routines are analyzed, when view and projection instructions are decoded, then the profiler extracts `camera_pointer_rva` and `projection_matrix_rva` and generates a typed `ClientCameraProfile`.
- [ ] Given dungeon window routines are analyzed, when global manager references are decoded, then the profiler extracts `runtime_state_pointer_rva` and generates a typed `ClientDungeonProfile`.
- [ ] Given the Setup Wizard executes stage 5 ("Client-Speicherprofile"), when the extractor runs, then the automated binary profiler executes automatically, writes the generated profile records to `data/config/` mapped to the executable's SHA-256 fingerprint, and reports a completed stage status.
- [ ] Given the desktop UI is open, when the operator clicks the "Profile neu analysieren / Re-scan Profiles" button in the Setup Wizard or Settings dialog, then the profiler re-runs against the selected client binary and updates the live readiness indicators immediately.
- [ ] Given an invalid, corrupt, or unresolvable executable is analyzed, when profiling cannot discover mandatory pointer RVAs, then the profiler fails closed, emits a localized diagnostic error, and prevents writing invalid or guessed memory offsets.
- [ ] All user-visible buttons, status messages, error logs, and tooltips are synchronized in German (`de.json`) and English (`en.json`).
