...
- In [US-023](US-023-reliable-combat-targeting-and-kill-verification.md), ground-truth kill tracking via `MonsterStatsReader` was established. Session progress and goals are better served by kill counts rather than fragile OCR-parsed item drop accounting.
...
- [x] **Direct combat-to-search transition (Auto-Looting):**
...
- [x] **Loot-Log OCR decoupling from default perception pipeline:**
  ...
  - Deferred to [US-022](US-022-modern-dark-theme-and-streamlined-dashboard-ui.md):
    the "Placements" ROI guide overlay does not exist yet; the loot-log region must be
    included there when that overlay is built.
- [x] **Farming goals and progress decoupling:**
  - Given session progress tracking, progress metrics and session completion are driven by mob kill verification ([US-023](US-023-reliable-combat-targeting-and-kill-verification.md)) rather than OCR inventory parsing.
...
- [x] **Safety boundaries and state consistency:**
...
- [x] **Localization and UI consistency:**
...
- Automated:
  ...
  - `./scripts/check.ps1` — this is PowerShell and could not run on the Linux implementation host; ran
    its five underlying commands directly instead (`uv sync --locked`, `uv run ruff check .`,
    `uv run ruff format --check .`, `uv run mypy`, `uv run pytest`). All pass except four
    pre-existing Windows-ctypes `mypy`/`pytest` failures in `input_control/controller.py` and
    `vision/capture.py`, confirmed identical on a stashed, unmodified `main` and therefore unrelated
    to this change.
- Manual (Windows):
