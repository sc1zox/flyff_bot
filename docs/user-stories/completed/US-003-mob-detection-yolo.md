...
- [x] Loads a YOLO model artifact from a specified file path with error handling for missing or corrupt files.
- [x] Performs object detection on input frames and returns structured detections containing bounding box (x, y, width, height), confidence score, class ID, and class name.
- [x] Supports configurable confidence threshold and class name filtering.
- [x] Returns empty detection lists gracefully without exceptions when no mobs are present.
- [x] Provides an abstraction/mock detector for fast unit tests without requiring GPU/live model files in standard test runs.
