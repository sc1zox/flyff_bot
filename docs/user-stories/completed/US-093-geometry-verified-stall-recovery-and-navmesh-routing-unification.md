...
- [x] **Removal of Hardcoded Evasion & `EVADING` Mode:** Given a stall is detected during navigation, when recovery is initiated, then no blind jump, turn, or backstep macros are executed; `PathingMode.EVADING` and `_evasion_steps` are removed and the controller remains logically in `PathingMode.TRAVELING`.
- [x] **Structured Stall Observation:** Given a commanded movement fails to produce the required GPS displacement within the stall timeout (`StallDetector`), when `PathingController` registers the stall, then it captures a typed `StallObservation` containing `previous_position`, `current_position`, `intended_direction`, `intended_waypoint`, `current_polygon_id`, and `timestamp`.
- [x] **Projected Temporary Obstacle Registry:** Given a `StallObservation`, when registering the obstacle in `TemporaryObstacleRegistry`, then the obstacle position is computed ahead along the intended direction (`player_position + intended_direction * obstacle_probe_distance`, default 1.2m) and projected onto the NavMesh (`nearest_walkable_position`), assigned a radius (default 1.5m) and dynamic TTL (15s for 1 hit, 30s for 2 hits, 60s for 3+ hits), without blocking the player's start coordinate.
- [x] **Temporary Obstacle Integration in `BakedNavMesh`:** Given active temporary obstacles, when `BakedNavMesh.find_path()` or `find_polygon_path()` computes a route, then intersecting polygons are excluded or penalized in A*, while protecting the start polygon from being hard-blocked.
- [x] **Immediate Local Replan on First Stall:** Given a first stall at an obstacle, when the temporary obstacle is registered, then the active route is invalidated and immediately re-planned via `BakedNavMesh.find_path()` with active temporary obstacles, resuming movement exclusively through standard `_steer()`.
- [x] **Repeated Stall Escalation & Local Stall History:** Given multiple stalls within a local spatial radius (default 2.0m) and sliding time window (default 10s), when the hit count reaches the threshold (`hit_count >= 2`), then the controller records `REPEATED_LOCAL_STALL` and escalates to the geometric escape planner (`_plan_escape_route`).
- [x] **Geometric Escape Planning & Validation:** Given repeated local stalls or an unroutable goal, when `_plan_escape_route()` executes, then it samples candidate points in concentric radial rings (e.g. 0.75m, 1.5m, 2.5m with 8–16 radial directions), validates each candidate on walkable NavMesh, checks slope limits, clearance, and obstacle distance, and scores candidates deterministically (goal progress, distance, clearance).
- [x] **Escape Execution via Standard Steering Pipeline:** Given a valid escape plan, when executing escape movement, then the chosen escape point is set as a temporary navigation waypoint and routed via `BakedNavMesh` and `_steer()`; upon reaching the escape point, standard routing to the original goal resumes automatically.
- [x] **Decommissioning of Legacy `TerrainRoutePlanner`:** Given `VectorZoneNavigator` and the navigation subsystem, when planning zone patrol sweeps and long-distance routes, then routes are planned directly over `BakedNavMesh.find_path()`; `terrain_routing.py` (`TerrainRoutePlanner`, `TerrainRouteConfig`, `TerrainWaypoint`) is completely removed.
- [x] **Telemetry & State Introspection:** Given stall and recovery events, when stepped, then `TelemetryRecorder` logs structured events (`STALL_DETECTED`, `TEMPORARY_OBSTACLE_CREATED`, `LOCAL_REPLAN_REQUESTED`, `LOCAL_REPLAN_SUCCEEDED`, `ESCAPE_PLAN_SUCCEEDED`, etc.); internal recovery status is tracked via `RecoveryContext` without exposing custom movement modes.
- [x] **Safety & Emergency Stop:** Given an emergency stop (`F12`), when triggered, then all movement, replanning, and recovery actions halt immediately and release input keys; all user-visible strings are synchronized in German and English.

## Implementation notes

- Recovery value objects, registries, and pure escape geometry live in
  `features/navigation/stall_recovery.py`: `StallObservation`, `TemporaryObstacle` /
  `TemporaryObstacleRegistry` (projection, merge, dynamic TTL), `RepeatedLocalStallTracker`,
  `RecoveryContext` / `RecoveryPhase` (internal only), and `plan_escape_candidates` /
  `select_escape`. `PathingController` orchestrates them from `_register_stall()`,
  `_plan_vector_route()`, and the rewritten `_plan_escape_route()`.
- `BakedNavMesh.find_path()` / `find_polygon_path()` take an `obstacles` tuple of
  `(centre, radius)` circles; `_polygon_ids_in_obstacles()` excludes intersecting polygons from
  `_a_star` (interior via `_height_at_xz`, edges via a 2D segment distance), and the start
  polygon is always removed from that set. `BakedNavMesh.surface_slope_degrees()` gives the
  escape planner a slope check without importing `telemetry`.
- `VectorZoneNavigator` no longer constructs a `TerrainRoutePlanner`; `_plan_leg()` routes only
  over the baked mesh and refuses every leg when no mesh is loaded, so a world with no baked
  NavMesh hard-blocks and the orchestrator pauses. `terrain_routing.py` and
  `test_terrain_routing.py` are deleted; `PathingController.__init__` now adopts the navigator's
  mesh so patrol, combat, and escape routing always agree.
- Telemetry adds seven `TelemetryEventKind` members and `TelemetryRecorder.record_stall_recovery_event()`;
  `PathingController.drain_recovery_events()` is pulled once per tick by
  `FarmingOrchestrator._forward_recovery_events()`. `NavigationEpisode.collision_evasions` /
  `evasion_seconds` are retained as generic recovery-action counters so the RL stack schema is
  unchanged.
...
- Automated:
  ```powershell
  uv run pytest tests/unit/test_vector_pathing.py tests/unit/test_vector_navigation.py tests/unit/test_navmesh_baker.py tests/unit/test_tracking.py tests/unit/test_telemetry.py tests/unit/test_orchestrator.py
  uv run ruff check .
