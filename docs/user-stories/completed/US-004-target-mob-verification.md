...
- [x] Extracts the target info region (e.g. top target bar area) from the captured game frame.
- [x] Inspects whether a valid target is currently selected.
- [x] Validates target attributes (e.g. target name match against configured whitelist/pattern, alive status via HP bar).
- [x] Returns a structured verification result (e.g. `TargetStatus.VALID_TARGET`, `TargetStatus.WRONG_TARGET`, `TargetStatus.NO_TARGET`).
- [x] Fast automated unit tests verify detection logic using cropped target bar fixtures (positive, negative, empty).
