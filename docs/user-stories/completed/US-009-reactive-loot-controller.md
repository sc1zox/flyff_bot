...
- Depends on [US-005](US-005-loot-log-ocr.md) (Loot OCR), [US-006](US-006-target-architecture-bootstrap.md) (Architecture/Controllers), and [US-007](US-007-perception-worldstate-feed.md).
- In Flyff, loot pickup is triggered by the pickup key/action (e.g. `F` or pet looting) after a mob dies.

## Acceptance criteria

- [x] `LootController` triggers pickup action sequence when a mob dies in combat.
- [x] Listens for `LootEvent` emissions from `LootLogReader` to record confirmed pickups.
- [x] Updates inventory counters and recipe progress metrics in `WorldState`.
- [x] Handles pickup timeout and resumes patrol if no items are received after configured wait duration.
- [x] Automated unit tests verify loot state machine transitions and drop accounting.
