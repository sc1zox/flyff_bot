...
- Therefore a loaded profile is silently reinterpreted relative to the position and facing the
  ...
  and rotate with it.
- [US-021](completed/US-021-navigation-map-profiles-and-session-reset.md) introduced named profile
...
- [ ] Given a session with a confident tracking quality, when a profile is saved, then the snapshot
...
- [ ] Given an anchored profile and a live minimap that overlaps the stored anchor above the
...
- [ ] Given an anchored profile whose stored scale metadata does not match the live scale, when the
...
- [ ] Given a profile is active, when the dashboard renders, then the anchor state (anchored /
