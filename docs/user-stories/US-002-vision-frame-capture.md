...
- [x] Captures the client area of the target Flyff window handle (`HWND`) into a standard image array (e.g. `numpy.ndarray`).
- [x] Provides an injectable frame-source interface so unit tests run deterministically with static fixtures.
- [x] Returns typed errors if the game window is minimized, invalid, occluded, or cannot be captured.
- [x] Frame coordinates accurately map to client-space pixels for subsequent click and bounding-box actions.
- [x] All user-visible error and log messages exist in German and English.
