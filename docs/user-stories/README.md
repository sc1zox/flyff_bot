...
- [ ] [**US-075: Portable one-click static client data extraction**](US-075-portable-client-static-data-extraction.md) — Installation-path-driven extraction of monsters, drops/AI, skills, items, economy, NPC actions, worlds/NavMesh, and quests into one portable dataset for another PC.
- [ ] [**US-076: Complete fingerprinted client player stats reader**](US-076-complete-client-player-stats-reader.md) — Profile-driven, bounded client-stat snapshots replace player-vitals OCR; verified x86/x64 field profiles remain required before live values are exposed.
...
- [ ] [**US-070: Learned attack point positioning and local waypoint optimization**](US-070-learned-attack-point-and-local-waypoint-optimization.md) — Sampling and scoring attack points within class engagement zones on the NavMesh to minimize approach time and position favorably for next spawns.
- [ ] [**US-071: Unified RL environment formulation, state-action space, and progress reward modeling**](US-071-unified-rl-environment-and-reward.md) — Standardized MDP observation space, discrete tactical actions, action masking, progress rewards, and telemetry transition exporter.
