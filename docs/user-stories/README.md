...
- [ ] [**US-037: Measured spawn distance model and an enforced patrol leash**](US-037-measured-spawn-distance-and-enforced-leash.md) — Replace the guessed bounding-box distance literals with a fitted inverse-projection relation and make `leash_radius_pixels` constrain planning instead of only the drawing.
