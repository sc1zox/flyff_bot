...
- [ ] [**US-092: Teleporter configuration, canonical target selection, closed-loop camera positioning, and legacy architecture pruning**](US-092-teleporter-config-target-selection-and-legacy-pruning.md) — Configurable teleporter hotkey, anchored dialog clicks, single source of truth target selection via canonical economic model, closed-loop camera positioning from memory, and decommissioning of legacy OCR and dead bootstrap stubs.
