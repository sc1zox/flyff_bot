...
- [x] [**US-077: Central live-state readiness gate refactor**](completed/US-077-central-live-state-readiness-gate.md) — Central typed readiness aggregation validates GPS, camera, player stats, dungeon state, perception freshness, and focus; affected capabilities pause coherently with localized diagnostics and safe recovery. Automated gate is green; Windows/live-client validation remains open.
...
- [x] [**US-079: Unified versioned goal-conditioned decision contract**](completed/US-079-unified-goal-conditioned-decision-contract.md) — One observation, parameterized action, mask and reward contract shared by simulator, exporter, trainer and live inference; duplicate action and goal vocabularies removed; goal identity, kind, sequence position and progress carried in the observation; one versioned reward configuration and a `us079-v1` contract stamp that every artifact and dataset carries and a foreign artifact is rejected against.
...
- [x] [**US-084: ML-modifiable tactical parameter space, hybrid tuning, and clamped safety bounds**](completed/US-084-ml-modifiable-tactical-parameters-and-tuning.md) — Typed 16-parameter space with bounded profile persistence, simulator/controller integration, prevalidated contextual approach overrides, deterministic clamping, localized diagnostics, and strict invariant isolation. Automated evidence is offline; live camera/zoom confirmation remains open.
