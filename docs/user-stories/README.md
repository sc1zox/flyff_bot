...
- [x] [**US-085: Production readiness, first-run wizard autostart, and autonomous farming polish**](completed/US-085-production-readiness-and-autonomous-farming-polish.md) — First-run wizard auto-detection on desktop startup, seamless dataset autoloading, production code assertion cleanup, robust autonomous farming loop with NavMesh/GPS pathing and combat reconciliation, 100% synchronized locales, and complete green quality gate.
- [x] [**US-086: Unattended autopilot mode, session resilience, and autonomous goal arbitration**](completed/US-086-unattended-autopilot-session-resilience-and-goal-arbitration.md) — Guarded tick loop with heartbeat and watchdog, player death state and respawn, emergency stop without `ESC`, graded fault recovery instead of session pause, deterministic goal arbitration with time and recovery budgets, and camera-projected quest NPC interaction.
...
- [x] [**US-089: Automated client binary reverse engineering and memory profiling**](completed/US-089-automated-client-binary-reverse-engineering-and-memory-profiling.md) — Offline binary reverse engineering of `neuz.exe` (PE headers, MSVC RTTI, VTables, instruction pattern matching) to statically derive exact pointer RVAs and struct offsets for position, camera, player stats, and dungeon lockout.
- [ ] [**US-088: Setup wizard autostart restriction, cached extraction, and UI re-extraction**](US-088-cached-setup-extraction-and-force-refresh.md) — Restrict Setup Wizard autostart to fresh installs without extracted data, add cached dataset skipping to avoid redundant I/O, and provide a full re-extraction toggle in the UI.
- [ ] [**US-090: World data dialog resizable layout and batch spawn zone selection**](US-090-world-data-dialog-resizable-layout-and-batch-spawn-zone-selection.md) — Resizable World Data dialog with dynamic vertical list expansion, geometry persistence, and batch "Select All" / "Deselect All" zone selection controls.
- [ ] [**US-091: Unified goal navigation, fluid scanning, and intelligent evasion recovery**](US-091-unified-goal-navigation-fluid-scanning-and-intelligent-unstuck.md) — Strict zone-constrained goal locking, off-zone mob suppression, fluid micro-sweeps without idle delays, elimination of blind keyboard roaming, and emergency stop compliance.
- [ ] [**US-092: Teleporter configuration, canonical target selection, closed-loop camera positioning, and legacy architecture pruning**](completed/US-092-teleporter-config-target-selection-and-legacy-pruning.md) — In progress: implemented configurable/anchored teleporting, canonical target ranking, closed-loop camera actuation, live-only stall detection, and dead-code pruning; vitals/nameplate decommissioning remain evidence-gated and live Windows validation is unrun.
- [ ] [**US-093: Geometry-verified stall recovery and NavMesh routing unification**](US-093-geometry-verified-stall-recovery-and-navmesh-routing-unification.md) — NavMesh-planned stall recovery with temporary obstacle projection and local escape routing, removal of blind evasion key macros, and unification of all routing under `BakedNavMesh` with legacy `TerrainRoutePlanner` removed.
- [x] [**US-094: Memory path for vital percentages via CWndStatus gauge floats**](completed/US-094-cwndstatus-gauge-vital-memory-path.md) — Feasibility spike for reading HP/MP/FP percentages from the `CWndStatus` gauge fill floats; closed as not implementable within ADR-006 (no fingerprint-stable anchor to the window instance), vitals stay on `PlayerVitalsReader` per ADR-010.
- [x] [**US-095: Initial Power-Up and Buff Dispatch on Session Start**](completed/US-095-initial-powerup-buff-dispatch-on-session-start.md) — Immediate initial dispatch of enabled power-ups and buff hotkeys upon session start with stagger delays, followed by periodic interval recurrence.
