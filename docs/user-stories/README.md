...
- [ ] [**US-092: Teleporter configuration, canonical target selection, closed-loop camera positioning, and legacy architecture pruning**](US-092-teleporter-config-target-selection-and-legacy-pruning.md) — In progress: implemented configurable/anchored teleporting, canonical target ranking, closed-loop camera actuation, live-only stall detection, and dead-code pruning; vitals/nameplate decommissioning remain evidence-gated and live Windows validation is unrun.
