...
sources:
  ...
  - ../sources/2026-08-29-entropia-cwndstatus-and-player-position-static-analysis.md
related:
  - project-overview.md
  - glossary.md
...
registries. Position discovery now decodes the proven `GetPlayer` -> helper -> 12-byte copy flow,
which yields the verified `CMover + 0x188` (`392`) for this x64 fingerprint; it no longer accepts
the former raw `48 05` byte-match false positive (`184`). The explicit x64 profile is committed in
`data/config/client_position_profiles.json`; there is no x86 entry because its offset has not been
independently verified. `LivePositionReader` uses only the exact-fingerprint registry entry and
has no embedded/default offset fallback. `data/config/client_world_id_profiles.json` is committed
empty because no world-ID offset is proven, so `LiveWorldIdReader` remains typed-unavailable and
arrival confirmation fails closed rather than guessing identity. See
[BUG-039](../bugs/fixed/BUG-039-generated-position-offset-false-positive-and-empty-position-world-id-registries.md).
