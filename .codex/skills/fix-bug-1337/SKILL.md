...
- **OPTIONAL**: Spawn the `researcher` subagent if you need to trace code paths across multiple files.
...
- **CONDITIONAL**: Spawn the `advisor` subagent if the bug touches Win32 handles, threading/Qt event loop, safety stop, or architecture boundaries.
...
- **MANDATORY**: Spawn the `verifier` subagent to run `./scripts/check.ps1` (uv sync, ruff check/format, mypy, pytest).
...
- **MANDATORY**: Spawn the `committer` subagent to create the commit for you (e.g., `fix(input): resolve key stuck state BUG-001`).
